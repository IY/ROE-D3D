...
HRESULT APIENTRY SetTexture_hook(LPDIRECT3DDEVICE9 pDevice, DWORD Sampler, IDirect3DBaseTexture9 *pTexture)
{
	...
	if (wallhack>0)
	{
		...
		//if ((vSize == 2356 || vSize == 900 || vSize == 2008 || vSize == 640) || (Stride == 72 && vSize == 1436) || (Stride == 48 && vSize == 1436)) //vSize == 600 //blood, gun shots & explosions
		if ((Stride == 72 && vSize > 1500 && vSize < 3100)) //vSize == 600 //blood, gun shots & explosions
		{
	...
	if ((aimbot == 1 || distanceesp == 1 || shaderesp == 1 || lineesp > 0 || boxesp == 1 || picesp == 1))
		if ((Stride == 72 && vSize > 1328) || (vSize == 2356 || vSize == 2008 || vSize == 1552)) // importante para o ESP
			AddWeapons(pDevice);

	//no fog for models
	if (nofog == 1)
		if (Stride == 72 || vSize == 2008 || vSize == 2356)
		{
			D3DXMATRIX matLegs;
	...
	out:
		...
		//green hair
		if ((Stride == 72 && vSize == 1836 && pSize != 1848 && pLockedRect.Pitch == 2048 && numElements == 6) || //hair	
			(Stride == 72 && vSize == 1836 && pSize == 2272 && pLockedRect.Pitch == 1024 && numElements == 10)) //hair2
		{
		...
		//red helmets
		if ((vSize == 1836 && qCRC == 0xc46ee841) ||//helmet 1
			(vSize == 1836 && qCRC == 0x9590d282) ||//helmet 2
			(vSize == 1836 && qCRC == 0xe248e914))//helmet 3
		{
...
HRESULT APIENTRY Present_hook(IDirect3DDevice9* pDevice, const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion)
{
	...
	if (aimkey == 1) Daimkey = VK_XBUTTON2;
	...
	if (shaderesp > 0 && WeaponEspInfo.size() != NULL)
	{
		for (unsigned int i = 0; i < WeaponEspInfo.size(); i++)
		{
			...
			if (WeaponEspInfo[i].pOutX > 1.0f && WeaponEspInfo[i].pOutY > 1.0f && (float)WeaponEspInfo[i].RealDistance > 4.0f)
				DX9DrawEllipse(pDevice, (float)WeaponEspInfo[i].pOutX - 6.0f, (float)WeaponEspInfo[i].pOutY - 8.0f, 16.0f, 32.0f, 1.0f, col);//- 8.0f, 16.0f, 32.0f, 1.0f
		}
	}
